...
await page.goto("http://localhost:3100/invite?token="+"a".repeat(72),{waitUntil:"networkidle"});
await page.getByRole("heading",{name:"Accept your invitation"}).waitFor();
await page.getByRole("link",{name:"Sign in to accept"}).waitFor();
assert.equal(await page.getByRole("button",{name:"Accept invitation"}).count(),0,"accept must not be offered while signed out");
assert.match(await page.getByRole("link",{name:"Sign in to accept"}).getAttribute("href"),/\/login\?next=%2Finvite%3Ftoken%3D/);
console.log("Signed-out invite offers Sign in to accept and keeps the token in the sign-in link");
