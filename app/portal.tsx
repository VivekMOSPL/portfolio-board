...
function Workspace({section}:{section:string}) {
 ...
 {modal&&<Dialog key={modal.action+s(modal.extra?.id)} modal={modal} onClose={()=>setModal(null)} onSubmit={async data=>{
  ...
  if(result.token)setNotice("Invitation created for manual delivery. Share this single-use link securely: "+window.location.origin+"/invite?token="+result.token);
...
function Team(p:ContentProps) {
 ...
 {p.member.role==="admin"&&<section className="panel"><h2>Invitations</h2><p className="muted">Manual link delivery is enabled. No email is sent by this screen. Create a replacement invitation after revoking an existing one.</p>{invites.error&&<Notice error>{invites.error}</Notice>}{invites.data?.rows.map(row=><article key={s(row.id)} className="record-row"><div><strong>{s(row.name)}</strong><p>{s(row.email)} · {s(row.role)}</p></div><span>{row.used_at?"Accepted":row.revoked_at?"Revoked":new Date(s(row.expires_at))<new Date()?"Expired":"Pending"}</span>{!row.used_at&&!row.revoked_at&&<button onClick={()=>{if(confirm("Revoke this invitation?"))void p.act("invite.revoke",{id:row.id});}}>Revoke</button>}</article>)}</section>}</>;
}
