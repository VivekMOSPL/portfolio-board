...
function Auth({section}:{section:string}) {
 ...
 {section==="invite"&&(session.loading?<Loading/>:session.error&&session.status!==401?<Notice error>{session.error}</Notice>:!signedIn?<><Notice>You were invited as <strong>{s(invitedEmail)||"the address this link was sent to"}</strong>. Choose a password to accept. If you already have an account for this address, enter its existing password instead. Invitations expire after 48 hours and can be used once.</Notice><Form fields={[{name:"password",label:"Password",type:"password",required:true,hint:"At least 8 characters. New here? This creates your account."}]} submit="Accept invitation" onSubmit={async d=>{const result=await api("/api/auth/accept-invite",{method:"POST",body:JSON.stringify({token,password:d.password})});router.replace("/dashboard?tenant="+result.tenant_id);router.refresh();}}/><p className="muted">Forgot the password for this address? <Link href="/forgot-password">Reset it</Link>.</p></>:<><Notice>Signed in as {s(session.data?.email)}. Accepting adds that account to the business that sent this invitation.</Notice><Form fields={[{name:"token",label:"Invitation token",required:true}]} values={{token}} submit="Accept invitation" onSubmit={async d=>{const result=await api("/api/command",{method:"POST",body:JSON.stringify({tenant:null,action:"invite.accept",data:d})});router.replace("/dashboard?tenant="+result.tenant_id);router.refresh();}}/><p className="muted">Not {s(session.data?.email)}? <Link href={signInHref}>Sign in with another account</Link>.</p></>)}
