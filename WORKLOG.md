...
Fixed the misleading invitation error (email mismatch reported as expired) -> node --import tsx --test tests/database.test.mjs; npm run db:migrate -> tests 25; pass 25; fail 0; migration 202609260006 applied; hosted cb_command raises "Sign in as <invited email> to accept this invitation" (42501) while unknown/expired/used tokens keep the generic message.
Made the invitation page session-aware and token-preserving -> npm run test:browser -> "Signed-out invite offers Sign in to accept and keeps the token in the sign-in link"; no Accept button while signed out; login/register return to the invite via ?next= (relative paths only).
Rewrote the invitation email for a recipient who may have no account, and added an HTML alternative -> npm run mail:verify -> decoded message carries recipient, sender, intact link, business, expiry, text + HTML parts, "Accept your invitation" CTA and account guidance; failure and unconfigured paths unchanged; npm test 33 pass.
